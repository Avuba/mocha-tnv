...
__private__.addProcess = function(number, queue, level) {
  var processVars = {
    env: {
      ...
      NODE_DBNAME: __private__.config.database + '-' + number,
  ...
  queue.addWorker(processVars.env.NODE_PROCESSNAME, function(fileName, done) {
    ...
    var template = 'var fs = require("fs"); var utilsPath = "{{ UTILS-PATH }}"; var tnv = {}; if (fs.existsSync(utilsPath)) tnv = require(utilsPath); var lodash = require("lodash"); var copyOfDescribe = lodash.cloneDeep(describe); describe = function(title, done) { copyOfDescribe(title, function() { return done.call(this, tnv); }); }; require("{{ FILENAME }}");';
